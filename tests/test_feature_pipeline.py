...
from src.feature_pipeline.clean import (
    ...
    _fix_zeros_with_medians,  # <--- UPDATED NAME
    run_cleaning,
    COLS_TO_DROP
)
...
def test_fix_zeros_imputation():
    ...
    df = pd.DataFrame({
        "median_list_price": [0, 500, 1000, 0], 
        "other_col": [1, 2, 3, 4]
    })
    
    # Define a mock artifact dictionary
    medians = {"median_list_price": 750}
    
    # CALL UPDATED FUNCTION NAME
    fixed_df = _fix_zeros_with_medians(df, medians)
    
    # Zeros should be replaced by 750
...
def test_frequency_encoding_logic():
    ...
    t_out, e_out, h_out, _ = _apply_frequency_encoding(train, eval_df, holdout)
    
    # Train: A->2, B->1
    assert t_out["zipcode_freq"].tolist() == [2, 2, 1]
    # Eval: A->2 (Known), C->0 (Unknown)
...
def test_full_pipeline_integration(mock_trans_artifacts, mock_clean_artifacts, tmp_path):
    ...
    Mocks ARTIFACTS_DIR so JSON/PKL files are saved to tmp_path instead of real models/ folder.
    """
    # 1. Setup Directories
    ...
    # Configure Mocks to point to temp artifacts folder
    ...
    df = pd.DataFrame({
        "date": pd.date_range("2018-01-01", periods=10, freq="180D"),
        "price": range(10),
        "zipcode": [1000, 1000, 2000, 1000, 2000, 3000, 1000, 2000, 3000, 4000],
        "median_list_price": [0, 100, 100, 100, 100, 100, 100, 100, 100, 100], 
        "median_sale_price": [99]*10 
    })
    ...
    assert "median_sale_price" not in train_proc.columns
